...
META = {
 "000": ("governance_classification", "P0", "governance", "Governance (this standard: contract layer only)"),
...
DOMAINS = {"governance": "MAINT-000..002", "technical": "MAINT-003..010",
           "operational": "MAINT-011..018", "evidence_control": "MAINT-019..024"}
...
def main():
    ...
    for nr in sorted(META):
        ...
        standards.append({
            ...
            "domain": domain, "priority_kai_os": prio,
    ...
    # 1) Conformance-SSOT
    doc = {"maintenance_conformance": {
        "standard": "ATC-STD-MAINT-000 \u00a720.1", "version": "1.0.0",
    ...
    doc2 = {"maintenance_requirements": {
        "standard": "ATC-STD-MAINT-000 \u00a76", "version": "1.0.0", "generated_note": GEN_NOTE,
    ...
    with io.open(OUT_REQ, "w", encoding="utf-8") as f:
        yaml.safe_dump(doc2, f, allow_unicode=True, sort_keys=False, default_flow_style=False, width=110)

    # 3) Family Index
    lines = ["# Maintenance Family Index (ATC-STD-MAINT-000 \u00a725)", "",
             "<!-- " + GEN_NOTE + " -->", "",
             f"**Parent Standard:** ATC-STD-MAINT-000 \u2014 normative Governance & Contract Layer.  ",
             ...
             f"**Conformance-SSOT:** registry/maintenance-conformance.yaml \u00b7 **Requirements-Registry:** registry/maintenance-requirements.yaml", "",
             "| ID | Rolle | Domain | Prio | Version | Status | REQs |", "|---|---|---|---|---|---|---|"]
    for s in standards:
        lines.append(f"| [{s['id']}](ATC-STD-MAINT-{s['id'][-3:]}.md) | {s['role']} | {s['domain']} | {s['priority_kai_os']} | {s['version']} | {s['status']} | {s['req_count']} |")
    lines += ["", f"Domains: " + " \u00b7 ".join(f"`{k}` ({v})" for k, v in DOMAINS.items()), ""]
    with io.open(OUT_IDX, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))

    p0 = [s["id"] for s in standards if s["priority_kai_os"] == "P0"]
    print(f"OK: conformance.yaml ({len(standards)} Standards, {len(p0)}xP0, {sum(s['req_count'] for s in standards)} REQs) "
