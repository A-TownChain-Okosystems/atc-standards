...
standard:
  id: ATC-STD-420
  title: "GameFi Architecture Standard"
  version: "1.2.0"
  status: approved
...
# ATC-STD-420 — GameFi Architecture (v1.2.0, APPROVED)

> **Status:** APPROVED (v1.2.0, §30-eingefroren) — Standard aus Katalog-Slot der Familie
> GameFi / Shivamon (FAM-25); §9-FREIGEGEBEN 08.09.2026, 02:15 UTC+2 — APPROVED, normativ,
...
ATC-STD-420 (GameFi Architecture) ist der Standard für den gleichnamigen Katalog-Slot der Familie
**GameFi / Shivamon** (FAM-25, Range ATC-STD-420..434) im ATC Enterprise Standards Framework. Er
...
**Gilt:** Der durch den Slot-Titel (GameFi Architecture) bezeichnete Gegenstandsbereich im
Zuständigkeitsfeld der Familie GameFi / Shivamon. **Gilt nicht:** Bereiche, die durch
...
Dieser Standard adressiert **GameFi Architecture** im Katalog-Slot ATC-STD-420 der Familie GameFi / Shivamon
(FAM-25); die Zuordnung folgt registry/framework.yaml (S-21) und darf nur via SCR
geändert werden.

GameFi-/Shivamon-Schicht: genesis-engine (L6, Vision-Dokument), deterministische Game-Logik, Ökonomie-Balancing.
...
1. **KR-1:** Game-Logik MUSS deterministisch sein (gleichbefindete Simulationen erzeugen gleiche Ergebnisse).
2. **KR-2:** Ökonomie-Parameter MÜSSEN simulierbar und balanciert sein (Inflation-Modelle ehrlich bewertet).
3. **KR-3:** Vision-Komponenten (ATC-41+) MÜSSEN von Engineering-Metriken getrennt bleiben (keine Sprint-ETAs aus Vision-Docs).
4. **KR-4:** On-Chain/Off-Chain-Trennung MUSS definiert sein (was landet auf der Chain).
5. **KR-5:** Anti-Cheat-Regeln MÜSSEN katalogisiert sein.
6. **KR-6:** Content-Updates DÜRFEN Konsens-Kern nicht brechen (MAJOR-Gate).
...
- **Nachbarfamilien:** Familie GameFi / Shivamon — Subsidiarität: konkretere Standards gehen vor.
- **Agenten-Bindung:** Vollmandat via .github/ai/agent.yaml; Umsetzungspflicht nach
  AGENT_MANIFEST.

## §4 Metriken & Akzeptanzkriterien

- **M1:** Determinismus-Simulationen identisch
- **M2:** Ökonomie-Modelle bewertet
- **M3:** 0 Vision-Metriken in Sprint-ETAs
...
## §6 Slot-Spezifikation GameFi Architecture — verbindliche Prüfkriterien
...
- **P1** (MUSS): Spielzustandsmodell und Determinismus — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P2** (MUSS): Wirtschafts-/Balancing-Parameter bewertet — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P3** (MUSS): Anti-Cheat-Regeln und Nachweis — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P4** (MUSS): On-/Off-Chain-Trennung definiert — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P5** (MUSS): Speicherung und Synchronisation — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P6** (MUSS): Familienkonformitaet: GameFi / Shivamon-Kernregeln KR-1..KR-6 eingehalten — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
...
- **REQ-STD-011** (§6/P1): Spielzustandsmodell und Determinismus
- **REQ-STD-012** (§6/P2): Wirtschafts-/Balancing-Parameter bewertet
- **REQ-STD-013** (§6/P3): Anti-Cheat-Regeln und Nachweis
- **REQ-STD-014** (§6/P4): On-/Off-Chain-Trennung definiert
- **REQ-STD-015** (§6/P5): Speicherung und Synchronisation
- **REQ-STD-016** (§6/P6): Familienkonformitaet: GameFi / Shivamon-Kernregeln KR-1..KR-6 eingehalten

## Security Considerations

Nicht deterministische Game-Logik bricht Konsens-Zustände; Ökonomie-Exploits durch unbewertete Parameter.
...
- **1.0.0** (2026-09-08): Initial Release — Grundgerüst-Standard aus
  Katalog-Slot ATC-STD-420 (Familie GameFi / Shivamon, FAM-25) via SCR-0030; §9-FREIGEGEBEN
...
- registry/framework.yaml (FAM-25), registry/standards.yaml + versions.yaml

*ATC-STD-420 v1.2.0 · Slot-Fertigbau via SCR-0034 · Aurora (Superagent) · 08.09.2026*
