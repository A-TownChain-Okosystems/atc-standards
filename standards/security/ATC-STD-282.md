...
standard:
  id: ATC-STD-282
  title: "Wallet Identity Standard"
  version: "1.2.0"
  status: approved
  category: security
...
# ATC-STD-282 — Wallet Identity (v1.2.0, APPROVED)

> **Status:** APPROVED (v1.2.0, §30-eingefroren) — Standard aus Katalog-Slot der Familie
> Identity & Reputation (FAM-17); §9-FREIGEGEBEN 08.09.2026, 02:15 UTC+2 — APPROVED, normativ,
...
ATC-STD-282 (Wallet Identity) ist der Standard für den gleichnamigen Katalog-Slot der Familie
**Identity & Reputation** (FAM-17, Range ATC-STD-280..286) im ATC Enterprise Standards Framework. Er
...
**Gilt:** Der durch den Slot-Titel (Wallet Identity) bezeichnete Gegenstandsbereich im
Zuständigkeitsfeld der Familie Identity & Reputation. **Gilt nicht:** Bereiche, die durch
...
Dieser Standard adressiert **Wallet Identity** im Katalog-Slot ATC-STD-282 der Familie Identity & Reputation
(FAM-17); die Zuordnung folgt registry/framework.yaml (S-21) und darf nur via SCR
geändert werden.

Identitäts-/Reputationsschicht: DID (Kernel K6), Ed25519 (K6b), Reputation (K15), RCT — Self-Sovereign Identity als Referenzmodell.
...
1. **KR-1:** Identitäten MÜSSEN kryptografisch an Schlüssel gebunden sein (DID-Dokument, Rotation definiert).
2. **KR-2:** Reputation MUSS aus nachvollziehbaren Signalen berechnet werden (Formel dokumentiert, Manipulationsresistenz bewertet).
3. **KR-3:** Schlüsselverlust MUSS einen definierten Recovery-Pfad haben (Social/Threshold).
4. **KR-4:** Identity-Daten DÜRFEN nicht ohne Zustimmung weitergegeben werden (Datenminimierung).
5. **KR-5:** Sperrungen (Banned/Revoked) MÜSSEN ökosystemweit konsistent propagieren.
6. **KR-6:** Anonymisierung vs. Accountability MUSS explizit ausbalanciert dokumentiert sein.
...
- **Nachbarfamilien:** Familie Identity & Reputation — Subsidiarität: konkretere Standards gehen vor.
- **Agenten-Bindung:** Vollmandat via .github/ai/agent.yaml; Umsetzungspflicht nach
  AGENT_MANIFEST.

## §4 Metriken & Akzeptanzkriterien

- **M1:** Key-Rotation ohne Dienstausfall
- **M2:** Reputation-Manipulationsresistenz bewertet
- **M3:** Recovery-Erfolgsquote
...
## §6 Slot-Spezifikation Wallet Identity — verbindliche Prüfkriterien
...
- **P1** (MUSS): Key-Management und HAL-Bindung — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P2** (MUSS): Signatur-Determinismus und Domain-Separation — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P3** (MUSS): Backup-/Recovery-Verfahren getestet — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P4** (MUSS): Sync mit Chain-Zustand — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P5** (MUSS): Fehlerzustands-Katalog — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P6** (MUSS): Identitaetsmodell (DID-Dokument, Schluesselbindung) — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P7** (MUSS): Authentisierungs- und Session-Regeln — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P8** (MUSS): Rotation-/Recovery-Prozess — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
...
- **REQ-STD-011** (§6/P1): Key-Management und HAL-Bindung
- **REQ-STD-012** (§6/P2): Signatur-Determinismus und Domain-Separation
- **REQ-STD-013** (§6/P3): Backup-/Recovery-Verfahren getestet
- **REQ-STD-014** (§6/P4): Sync mit Chain-Zustand
- **REQ-STD-015** (§6/P5): Fehlerzustands-Katalog
- **REQ-STD-016** (§6/P6): Identitaetsmodell (DID-Dokument, Schluesselbindung)
- **REQ-STD-017** (§6/P7): Authentisierungs- und Session-Regeln
- **REQ-STD-018** (§6/P8): Rotation-/Recovery-Prozess

## Security Considerations

Identitätsdiebstahl durch schwache Rotation; Reputation-Gaming durch Sybil-/Collusion-Angriffe.
...
- **1.0.0** (2026-09-08): Initial Release — Grundgerüst-Standard aus
  Katalog-Slot ATC-STD-282 (Familie Identity & Reputation, FAM-17) via SCR-0030; §9-FREIGEGEBEN
...
- registry/framework.yaml (FAM-17), registry/standards.yaml + versions.yaml

*ATC-STD-282 v1.2.0 · Slot-Fertigbau via SCR-0034 · Aurora (Superagent) · 08.09.2026*
