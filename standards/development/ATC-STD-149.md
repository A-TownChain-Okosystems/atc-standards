...
standard:
  id: ATC-STD-149
  title: "Security Testing Standard"
  version: "1.2.0"
  status: approved
...
# ATC-STD-149 — Security Testing (v1.2.0, APPROVED)

> **Status:** APPROVED (v1.2.0, §30-eingefroren) — Standard aus Katalog-Slot der Familie
> Testing & Quality Assurance (FAM-08); §9-FREIGEGEBEN 08.09.2026, 02:15 UTC+2 — APPROVED, normativ,
...
ATC-STD-149 (Security Testing) ist der Standard für den gleichnamigen Katalog-Slot der Familie
**Testing & Quality Assurance** (FAM-08, Range ATC-STD-140..153) im ATC Enterprise Standards Framework. Er
...
**Gilt:** Der durch den Slot-Titel (Security Testing) bezeichnete Gegenstandsbereich im
Zuständigkeitsfeld der Familie Testing & Quality Assurance. **Gilt nicht:** Bereiche, die durch
...
Dieser Standard adressiert **Security Testing** im Katalog-Slot ATC-STD-149 der Familie Testing & Quality Assurance
(FAM-08); die Zuordnung folgt registry/framework.yaml (S-21) und darf nur via SCR
geändert werden.

Qualitätssicherung: Test-Suiten je Modul, QEMU-Verifikation für Kernel-Sprints, Mutationstest-Suite im Validator (S-19), FAIL-scharfe Konformanztests.
...
1. **KR-1:** Tests MÜSSEN deterministisch sein (keine Flakiness ohne Kennzeichnung).
2. **KR-2:** Grenzfälle und Fehlerpfade MÜSSEN getestet sein, nicht nur Happy Paths.
3. **KR-3:** Konformanzprüfungen MÜSSEN FAIL-scharf sein: ein Regelverstoß MUSS zum Fehler führen.
4. **KR-4:** Testergebnisse MÜSSEN maschinell reproduzierbar dokumentiert sein.
5. **KR-5:** Regressionstests MÜSSEN bei jedem Fix ergänzt werden.
6. **KR-6:** Coverage-Lücken MÜSSEN ehrlich benannt werden (keine geschönten Quoten).
...
- **Nachbarfamilien:** Familie Testing & Quality Assurance — Subsidiarität: konkretere Standards gehen vor.
...
- **M1:** Tests grün (X/X)
- **M2:** Mutationstest-Trefferquote
- **M3:** 0 ungetestete Fehlerpfade laut Doku
...
## §6 Slot-Spezifikation Security Testing — verbindliche Prüfkriterien
...
- **P1** (MUSS): Testebenen (Unit/Integration/Konformanz) — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P2** (MUSS): Determinismus und Reproduzierbarkeit — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P3** (MUSS): Fehlerpfad- und Grenzfallabdeckung — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P4** (MUSS): Metrikendokumentation (X/X gruen) — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P5** (MUSS): Regressionsschutz bei Fixes — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P6** (MUSS): Bedrohungskatalog je Gegenstand — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P7** (MUSS): Schutzmassnahmen mit Wirksamkeitsnachweis — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P8** (MUSS): Ehrlicher Umsetzungsstatus (ACTIVE/PARTIAL/PLANNED) — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
...
- **REQ-STD-011** (§6/P1): Testebenen (Unit/Integration/Konformanz)
- **REQ-STD-012** (§6/P2): Determinismus und Reproduzierbarkeit
- **REQ-STD-013** (§6/P3): Fehlerpfad- und Grenzfallabdeckung
- **REQ-STD-014** (§6/P4): Metrikendokumentation (X/X gruen)
- **REQ-STD-015** (§6/P5): Regressionsschutz bei Fixes
- **REQ-STD-016** (§6/P6): Bedrohungskatalog je Gegenstand
- **REQ-STD-017** (§6/P7): Schutzmassnahmen mit Wirksamkeitsnachweis
- **REQ-STD-018** (§6/P8): Ehrlicher Umsetzungsstatus (ACTIVE/PARTIAL/PLANNED)

## Security Considerations

Grün-Getestete Oberfläche mit ungetesteten Kernpfaden; Flaky-Tests verwässern das Vertrauen in die Pipeline.
...
- **1.0.0** (2026-09-08): Initial Release — Grundgerüst-Standard aus
  Katalog-Slot ATC-STD-149 (Familie Testing & Quality Assurance, FAM-08) via SCR-0030; §9-FREIGEGEBEN
...
- registry/framework.yaml (FAM-08), registry/standards.yaml + versions.yaml

*ATC-STD-149 v1.2.0 · Slot-Fertigbau via SCR-0034 · Aurora (Superagent) · 08.09.2026*
