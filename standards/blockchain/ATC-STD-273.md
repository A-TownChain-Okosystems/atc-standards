...
standard:
  id: ATC-STD-273
  title: "Oracle Consensus Standard"
  version: "1.2.0"
  status: approved
  category: blockchain
...
# ATC-STD-273 — Oracle Consensus (v1.2.0, APPROVED)

> **Status:** APPROVED (v1.2.0, §30-eingefroren) — Standard aus Katalog-Slot der Familie
> Oracle & External Data (FAM-16); §9-FREIGEGEBEN 08.09.2026, 02:15 UTC+2 — APPROVED, normativ,
...
ATC-STD-273 (Oracle Consensus) ist der Standard für den gleichnamigen Katalog-Slot der Familie
**Oracle & External Data** (FAM-16, Range ATC-STD-270..276) im ATC Enterprise Standards Framework. Er
...
**Gilt:** Der durch den Slot-Titel (Oracle Consensus) bezeichnete Gegenstandsbereich im
Zuständigkeitsfeld der Familie Oracle & External Data. **Gilt nicht:** Bereiche, die durch
...
Dieser Standard adressiert **Oracle Consensus** im Katalog-Slot ATC-STD-273 der Familie Oracle & External Data
(FAM-16); die Zuordnung folgt registry/framework.yaml (S-21) und darf nur via SCR
geändert werden.

Oracle-/Externe-Daten-Schicht (atc-oracle): signierte Datenfeeds in die Contract-/Blockchain-Schicht.
...
1. **KR-1:** Externe Daten MÜSSEN herkunfts-signiert (Ed25519/DID-basiert) in das Ökosystem gelangen.
2. **KR-2:** Oracle-Feeds MÜSSEN Replay-geschützt sein (Nonce/Timestamp/Chain-ID).
3. **KR-3:** Feed-Ausfälle MÜSSEN definierte Fehlerzustände erzeugen (keine stillen Alt-Daten-Verwendungen).
4. **KR-4:** Datenherkünfte MÜSSEN mit Quellen- und Vertrauensgrad registriert sein.
5. **KR-5:** Manipulationsschwellen MÜSSEN definiert sein (Abweichungs-/Ausfall-Alarme).
6. **KR-6:** Oracle-Konfiguration ist MAJOR-gebunden (COMPAT-001).
...
- **Nachbarfamilien:** Familie Oracle & External Data — Subsidiarität: konkretere Standards gehen vor.
- **Agenten-Bindung:** Vollmandat via .github/ai/agent.yaml; Umsetzungspflicht nach
  AGENT_MANIFEST.

## §4 Metriken & Akzeptanzkriterien

- **M1:** Feed-Verfügbarkeit
- **M2:** 0 unsignierte Datenannahmen
- **M3:** Ausfall-Erkennungszeit
...
## §6 Slot-Spezifikation Oracle Consensus — verbindliche Prüfkriterien
...
- **P1** (MUSS): Konsensalgorithmus-Parameter und -Grenzen — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P2** (MUSS): Validator-Zulaassungs-/Rotationsregeln — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P3** (MUSS): Voting-/Finality-Nachweise — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P4** (MUSS): Fork-/Divergenzbehandlung — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P5** (MUSS): DoS-/Manipulationsresistenz — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P6** (MUSS): Feed-Quellen mit Vertrauensgrad — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P7** (MUSS): Signatur- und Replay-Schutz — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P8** (MUSS): Ausfall-/Manipulations-Schwellen — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
...
- **REQ-STD-011** (§6/P1): Konsensalgorithmus-Parameter und -Grenzen
- **REQ-STD-012** (§6/P2): Validator-Zulaassungs-/Rotationsregeln
- **REQ-STD-013** (§6/P3): Voting-/Finality-Nachweise
- **REQ-STD-014** (§6/P4): Fork-/Divergenzbehandlung
- **REQ-STD-015** (§6/P5): DoS-/Manipulationsresistenz
- **REQ-STD-016** (§6/P6): Feed-Quellen mit Vertrauensgrad
- **REQ-STD-017** (§6/P7): Signatur- und Replay-Schutz
- **REQ-STD-018** (§6/P8): Ausfall-/Manipulations-Schwellen

## Security Considerations

Feed-Manipulation; Replay alter Preise/Daten ohne Zeit-/Nonce-Schutz.
...
- **1.0.0** (2026-09-08): Initial Release — Grundgerüst-Standard aus
  Katalog-Slot ATC-STD-273 (Familie Oracle & External Data, FAM-16) via SCR-0030; §9-FREIGEGEBEN
...
- registry/framework.yaml (FAM-16), registry/standards.yaml + versions.yaml

*ATC-STD-273 v1.2.0 · Slot-Fertigbau via SCR-0034 · Aurora (Superagent) · 08.09.2026*
