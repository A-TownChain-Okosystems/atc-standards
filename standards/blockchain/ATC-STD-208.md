...
standard:
  id: ATC-STD-208
  title: "Governance Token Standard"
  version: "1.2.0"
  status: approved
  category: blockchain
...
# ATC-STD-208 — Governance Token (v1.2.0, APPROVED)

> **Status:** APPROVED (v1.2.0, §30-eingefroren) — Standard aus Katalog-Slot der Familie
> Token Standards (FAM-12); §9-FREIGEGEBEN 08.09.2026, 02:15 UTC+2 — APPROVED, normativ,
...
ATC-STD-208 (Governance Token) ist der Standard für den gleichnamigen Katalog-Slot der Familie
**Token Standards** (FAM-12, Range ATC-STD-200..211) im ATC Enterprise Standards Framework. Er
...
**Gilt:** Der durch den Slot-Titel (Governance Token) bezeichnete Gegenstandsbereich im
Zuständigkeitsfeld der Familie Token Standards. **Gilt nicht:** Bereiche, die durch
...
Dieser Standard adressiert **Governance Token** im Katalog-Slot ATC-STD-208 der Familie Token Standards
(FAM-12); die Zuordnung folgt registry/framework.yaml (S-21) und darf nur via SCR
geändert werden.

Token-Schicht auf der Contract-Ebene (ATCLang, atc-contracts): deterministische Token-Formate, Kompatibilitäts-Matrix-Pflicht.
...
1. **KR-1:** Token-Operationen MÜSSEN deterministisch sein (Reihenfolge, Rundung, Überlauf-Verhalten definiert).
2. **KR-2:** Token-Standards MÜSSEN eine Kompatibilitätsmatrix je Version führen.
3. **KR-3:** Werttransport MUSS protokolliert werden (AuditTrail-Anbindung).
4. **KR-4:** Contract-Änderungen erfordern ein Audit-Gate vor Aktivierung.
5. **KR-5:** Token-Metadaten MÜSSEN versioniert und migrierbar sein.
6. **KR-6:** Supply-Regeln MÜSSEN maschinell prüfbar definiert sein (Mint/Burn-Grenzen).
...
- **Nachbarfamilien:** Familie Token Standards — Subsidiarität: konkretere Standards gehen vor.
- **Agenten-Bindung:** Vollmandat via .github/ai/agent.yaml; Umsetzungspflicht nach
  AGENT_MANIFEST.

## §4 Metriken & Akzeptanzkriterien

- **M1:** 0 Rundungs-/Überlauf-Divergenzen
- **M2:** Matrix-Abdeckung 100 %
- **M3:** Audit-Gate erfüllt
...
## §6 Slot-Spezifikation Governance Token — verbindliche Prüfkriterien
...
- **P1** (MUSS): Entscheidungs- und Freigabeprozesse mit Human-Gate — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P2** (MUSS): Rollen/Verantwortlichkeiten (RACI) je Entscheidungstyp — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P3** (MUSS): Nachweispflicht (AUD-Record/Commit) und Datierung — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P4** (MUSS): Eskalations- und Konfliktloesungsweg — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P5** (MUSS): Wirksame Daten und Lifecycle (Inkraft/Ausserkraft) — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P6** (MUSS): Tokenmodell und Supply-Regeln — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P7** (MUSS): Operationen (Mint/Burn/Transfer) mit Grenzen — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P8** (MUSS): Determinismus (Rundung/Ueberlauf) — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
...
- **REQ-STD-011** (§6/P1): Entscheidungs- und Freigabeprozesse mit Human-Gate
- **REQ-STD-012** (§6/P2): Rollen/Verantwortlichkeiten (RACI) je Entscheidungstyp
- **REQ-STD-013** (§6/P3): Nachweispflicht (AUD-Record/Commit) und Datierung
- **REQ-STD-014** (§6/P4): Eskalations- und Konfliktloesungsweg
- **REQ-STD-015** (§6/P5): Wirksame Daten und Lifecycle (Inkraft/Ausserkraft)
- **REQ-STD-016** (§6/P6): Tokenmodell und Supply-Regeln
- **REQ-STD-017** (§6/P7): Operationen (Mint/Burn/Transfer) mit Grenzen
- **REQ-STD-018** (§6/P8): Determinismus (Rundung/Ueberlauf)

## Security Considerations

Supply-Drift durch undefinierte Rundung/Überlauf; nicht migrierbare Metadaten brechen Upgrade-Pfade.
...
- **1.0.0** (2026-09-08): Initial Release — Grundgerüst-Standard aus
  Katalog-Slot ATC-STD-208 (Familie Token Standards, FAM-12) via SCR-0030; §9-FREIGEGEBEN
...
- registry/framework.yaml (FAM-12), registry/standards.yaml + versions.yaml

*ATC-STD-208 v1.2.0 · Slot-Fertigbau via SCR-0034 · Aurora (Superagent) · 08.09.2026*
