...
standard:
  id: ATC-STD-349
  title: "Mining Security Standard"
  version: "1.2.0"
  status: approved
  category: blockchain
...
# ATC-STD-349 — Mining Security (v1.2.0, APPROVED)

> **Status:** APPROVED (v1.2.0, §30-eingefroren) — Standard aus Katalog-Slot der Familie
> Mining Standards (FAM-21); §9-FREIGEGEBEN 08.09.2026, 02:15 UTC+2 — APPROVED, normativ,
...
ATC-STD-349 (Mining Security) ist der Standard für den gleichnamigen Katalog-Slot der Familie
**Mining Standards** (FAM-21, Range ATC-STD-340..350) im ATC Enterprise Standards Framework. Er
...
**Gilt:** Der durch den Slot-Titel (Mining Security) bezeichnete Gegenstandsbereich im
...
Dieser Standard adressiert **Mining Security** im Katalog-Slot ATC-STD-349 der Familie Mining Standards
(FAM-21); die Zuordnung folgt registry/framework.yaml (S-21) und darf nur via SCR
geändert werden.

Mining-/Validierungsschicht: Kernel K16 (DAG+PoH+Validator+Voting+Finality), Rate-Limits (K15), Validator-Rotation.
...
1. **KR-1:** Mining-/Validierungsregeln MÜSSEN deterministisch sein (kein Nichtdeterminismus im Reward-Pfad).
2. **KR-2:** Validator-Zulassung/-Rotation MUSS nach dokumentierten Kriterien erfolgen.
3. **KR-3:** Rate-Limits MÜSSEN die Ressourcen schützen (DoS-Resistenz bewertet).
4. **KR-4:** Reward-Berechnung MUSS testbar und auditierbar sein (Regressionstests).
5. **KR-5:** Angriffe auf den Validierungsprozess MÜSSEN im Threat-Model katalogisiert sein.
6. **KR-6:** Änderungen an Belohnungs-/Validierungsparametern sind MAJOR (COMPAT-001).
...
- **Nachbarfamilien:** Familie Mining Standards — Subsidiarität: konkretere Standards gehen vor.
- **Agenten-Bindung:** Vollmandat via .github/ai/agent.yaml; Umsetzungspflicht nach
  AGENT_MANIFEST.

## §4 Metriken & Akzeptanzkriterien

- **M1:** 0 nicht deterministische Reward-Pfade
- **M2:** DoS-Resistenz nachgewiesen
- **M3:** Rotations-Nachweise
...
## §6 Slot-Spezifikation Mining Security — verbindliche Prüfkriterien
...
- **P1** (MUSS): Validierungs-/Reward-Regeln deterministisch — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P2** (MUSS): Zulaassungs- und Rotationskriterien — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P3** (MUSS): Ressourcenschutz (Rate-Limits) — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P4** (MUSS): Reward-Nachweisbarkeit und Regressionstests — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P5** (MUSS): MAJOR-Gate fuer Parameter — Nachweis: Design-/Konzeptdokument + AUD-Record
...
- **REQ-STD-011** (§6/P1): Validierungs-/Reward-Regeln deterministisch
- **REQ-STD-012** (§6/P2): Zulaassungs- und Rotationskriterien
- **REQ-STD-013** (§6/P3): Ressourcenschutz (Rate-Limits)
- **REQ-STD-014** (§6/P4): Reward-Nachweisbarkeit und Regressionstests
- **REQ-STD-015** (§6/P5): MAJOR-Gate fuer Parameter
...
Nothing-at-Stake/Long-Range-Angriffe; Parameter-Manipulation zugunsten einzelner Validatoren.
...
- **1.0.0** (2026-09-08): Initial Release — Grundgerüst-Standard aus
  Katalog-Slot ATC-STD-349 (Familie Mining Standards, FAM-21) via SCR-0030; §9-FREIGEGEBEN
...
- registry/framework.yaml (FAM-21), registry/standards.yaml + versions.yaml

*ATC-STD-349 v1.2.0 · Slot-Fertigbau via SCR-0034 · Aurora (Superagent) · 08.09.2026*
