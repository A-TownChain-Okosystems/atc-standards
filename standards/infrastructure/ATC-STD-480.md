...
# ATC-STD-480 — Observability (v1.2.0, APPROVED)

> **Status:** APPROVED (v1.2.0, §30-eingefroren) — Standard aus Katalog-Slot der Familie
> Observability (FAM-28); §9-FREIGEGEBEN 08.09.2026, 02:15 UTC+2 — APPROVED, normativ,
...
ATC-STD-480 (Observability) ist der Standard für den gleichnamigen Katalog-Slot der Familie
**Observability** (FAM-28, Range ATC-STD-480..488) im ATC Enterprise Standards Framework. Er
...
**Gilt:** Der durch den Slot-Titel (Observability) bezeichnete Gegenstandsbereich im
Zuständigkeitsfeld der Familie Observability. **Gilt nicht:** Bereiche, die durch
...
Dieser Standard adressiert **Observability** im Katalog-Slot ATC-STD-480 der Familie Observability
(FAM-28); die Zuordnung folgt registry/framework.yaml (S-21) und darf nur via SCR
geändert werden.

Observability-Schicht: Kernel security_audit (K15), strukturierte Logs, Metriken je Subsystem.
...
1. **KR-1:** Betriebsdaten MÜSSEN strukturiert (Schema, Level, Kontext) erhoben werden.
2. **KR-2:** Kritische Ereignisse MÜSSEN unveränderlich protokolliert sein (AuditTrail-Kopplung).
3. **KR-3:** Metriken MÜSSEN definierte Schwellen mit Alarmierung haben.
4. **KR-4:** Health-/Statusberichte MÜSSEN automatisch generierbar sein.
5. **KR-5:** Log-Flut MUSS begrenzt sein (Retention, Sampling dokumentiert).
6. **KR-6:** Sensible Daten DÜRFEN nicht in Logs landen (Redaktionsregeln).
...
- **Nachbarfamilien:** Familie Observability — Subsidiarität: konkretere Standards gehen vor.
- **Agenten-Bindung:** Vollmandat via .github/ai/agent.yaml; Umsetzungspflicht nach
  AGENT_MANIFEST.

## §4 Metriken & Akzeptanzkriterien

- **M1:** Alert-Treue (0 False Negatives kritisch)
- **M2:** Log-Strukturkonformanz
- **M3:** Metrik-Schwellen konfiguriert
...
## §6 Slot-Spezifikation Observability — verbindliche Prüfkriterien
...
- **P1** (MUSS): Metrik-/Log-Schema je Subsystem — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P2** (MUSS): Schwellenwerte und Alarmierung — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P3** (MUSS): Unveraenderlichkeit kritischer Ereignisse — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P4** (MUSS): Retention-/Sampling-Regeln — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P5** (MUSS): Vertraulichkeit (Redaktion) — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P6** (MUSS): Familienkonformitaet: Observability-Kernregeln KR-1..KR-6 eingehalten — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
...
- **REQ-STD-011** (§6/P1): Metrik-/Log-Schema je Subsystem
- **REQ-STD-012** (§6/P2): Schwellenwerte und Alarmierung
- **REQ-STD-013** (§6/P3): Unveraenderlichkeit kritischer Ereignisse
- **REQ-STD-014** (§6/P4): Retention-/Sampling-Regeln
- **REQ-STD-015** (§6/P5): Vertraulichkeit (Redaktion)
- **REQ-STD-016** (§6/P6): Familienkonformitaet: Observability-Kernregeln KR-1..KR-6 eingehalten

## Security Considerations

Alarm-Ermüdung durch Rauschen; Beweisverlust durch rotierte/ungefilterte Logs.
...
- **1.0.0** (2026-09-08): Initial Release — Grundgerüst-Standard aus
  Katalog-Slot ATC-STD-480 (Familie Observability, FAM-28) via SCR-0030; §9-FREIGEGEBEN
...
- registry/framework.yaml (FAM-28), registry/standards.yaml + versions.yaml

*ATC-STD-480 v1.2.0 · Slot-Fertigbau via SCR-0034 · Aurora (Superagent) · 08.09.2026*
