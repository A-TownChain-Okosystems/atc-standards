...
# ATC-STD-680 — Open Source (v1.2.0, APPROVED)

> **Status:** APPROVED (v1.2.0, §30-eingefroren) — Standard aus Katalog-Slot der Familie
> Open Source & Lizenzierung (FAM-38); §9-FREIGEGEBEN 08.09.2026, 02:15 UTC+2 — APPROVED, normativ,
...
ATC-STD-680 (Open Source) ist der Standard für den gleichnamigen Katalog-Slot der Familie
**Open Source & Lizenzierung** (FAM-38, Range ATC-STD-680..686) im ATC Enterprise Standards Framework. Er
...
**Gilt:** Der durch den Slot-Titel (Open Source) bezeichnete Gegenstandsbereich im
Zuständigkeitsfeld der Familie Open Source & Lizenzierung. **Gilt nicht:** Bereiche, die durch
...
Dieser Standard adressiert **Open Source** im Katalog-Slot ATC-STD-680 der Familie Open Source & Lizenzierung
(FAM-38); die Zuordnung folgt registry/framework.yaml (S-21) und darf nur via SCR
geändert werden.

Open-Source-/Lizenzschicht: Copyright-Name „Michael Wroblewski“ verbindlich (>500 Vorkommen), Lizenz-Konsistenz über alle Repos.
...
1. **KR-1:** Der Copyright-Name MUSS unverändert „Michael Wroblewski“ bleiben.
2. **KR-2:** Lizenzen MÜSSEN je Repo eindeutig deklariert sein (LICENSE-Datei).
3. **KR-3:** Lizenz-Header MÜSSEN konsistent sein (Automatisierung prüfbar).
4. **KR-4:** Drittanbieter-Lizenzen MÜSSEN erfasst und kompatibel sein (Pflichten dokumentiert).
5. **KR-5:** Lizenzwechsel sind Owner-Entscheidungen (MAJOR, dokumentiert).
6. **KR-6:** Contributions MÜSSEN den Lizenzbedingungen zugeordnet werden können.
...
- **Nachbarfamilien:** Familie Open Source & Lizenzierung — Subsidiarität: konkretere Standards gehen vor.
- **Agenten-Bindung:** Vollmandat via .github/ai/agent.yaml; Umsetzungspflicht nach
  AGENT_MANIFEST.

## §4 Metriken & Akzeptanzkriterien

- **M1:** Lizenz-Deklaration 100 %
- **M2:** Header-Konsistenz 100 %
- **M3:** 0 inkompatible Dritt-Lizenzen
...
## §6 Slot-Spezifikation Open Source — verbindliche Prüfkriterien
...
- **P1** (MUSS): Lizenzdeklaration je Repo — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P2** (MUSS): Header-Konsistenz (Copyright Wroblewski) — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P3** (MUSS): Dritt-/Copyleft-Kompatibilitaet geprueft — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P4** (MUSS): Lizenzwechsel nur Owner (MAJOR) — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
- **P5** (MUSS): Contributions lizenzzuordenbar — Nachweis: Design-/Konzeptdokument + AUD-Record
- **P6** (MUSS): Familienkonformitaet: Open Source & Lizenzierung-Kernregeln KR-1..KR-6 eingehalten — Nachweis: Validator-/Testlauf bzw. dokumentierte Prüfung
...
- **REQ-STD-011** (§6/P1): Lizenzdeklaration je Repo
- **REQ-STD-012** (§6/P2): Header-Konsistenz (Copyright Wroblewski)
- **REQ-STD-013** (§6/P3): Dritt-/Copyleft-Kompatibilitaet geprueft
- **REQ-STD-014** (§6/P4): Lizenzwechsel nur Owner (MAJOR)
- **REQ-STD-015** (§6/P5): Contributions lizenzzuordenbar
- **REQ-STD-016** (§6/P6): Familienkonformitaet: Open Source & Lizenzierung-Kernregeln KR-1..KR-6 eingehalten

## Security Considerations

Lizenz-Inkonsistenzen erzeugen Rechtsrisiken; still eingeführte Copyleft-Abhängigkeiten kontaminieren Projekte.
...
- **1.0.0** (2026-09-08): Initial Release — Grundgerüst-Standard aus
  Katalog-Slot ATC-STD-680 (Familie Open Source & Lizenzierung, FAM-38) via SCR-0030; §9-FREIGEGEBEN
...
- registry/framework.yaml (FAM-38), registry/standards.yaml + versions.yaml

*ATC-STD-680 v1.2.0 · Slot-Fertigbau via SCR-0034 · Aurora (Superagent) · 08.09.2026*
