...
# ATC-STD-640 — AuditTrail (v1.2.0, APPROVED)

> **Status:** APPROVED (v1.2.0, §30-eingefroren) — Standard aus Katalog-Slot der Familie
> AuditTrail / LogChain (FAM-36); §9-FREIGEGEBEN 08.09.2026, 02:15 UTC+2 — APPROVED, normativ,
...
ATC-STD-640 (AuditTrail) ist der Standard für den gleichnamigen Katalog-Slot der Familie
**AuditTrail / LogChain** (FAM-36, Range ATC-STD-640..646) im ATC Enterprise Standards Framework. Er
...
**Gilt:** Der durch den Slot-Titel (AuditTrail) bezeichnete Gegenstandsbereich im
Zuständigkeitsfeld der Familie AuditTrail / LogChain. **Gilt nicht:** Bereiche, die durch
...
Dieser Standard adressiert **AuditTrail** im Katalog-Slot ATC-STD-640 der Familie AuditTrail / LogChain
(FAM-36); die Zuordnung folgt registry/framework.yaml (S-21) und darf nur via SCR
geändert werden.

AuditTrail-/LogChain-Schicht: Kernel security_audit (K15), AUD-Record-Pflicht (AI-DEV-009), unveränderliche Protokollketten.

**Katalog-Referenz:** Kernel K15 security_audit als Referenz

## §2 Kernregeln (familienweit, elaboriert)

1. **KR-1:** Auditrelevante Ereignisse MÜSSEN unveränderlich protokolliert sein (Append-only, ggf. hashverkettet).
2. **KR-2:** AUD-Records MÜSSEN die Pflichtfelder (WER/WAS/WANN/Nachweis) vollständig führen.
3. **KR-3:** Protokollierung MUSS strukturiert und abfragbar sein.
4. **KR-4:** Manipulationsversuche an Protokollen MÜSSEN erkennbar sein (Integritätsprüfung).
5. **KR-5:** Aufbewahrungs-/Zugriffsregeln MÜSSEN definiert sein.
6. **KR-6:** Kettenkopplung an die Blockchain MUSS deterministisch erfolgen.
...
- **Nachbarfamilien:** Familie AuditTrail / LogChain — Subsidiarität: konkretere Standards gehen vor.
- **Agenten-Bindung:** Vollmandat via .github/ai/agent.yaml; Umsetzungspflicht nach
  AGENT_MANIFEST.

## §4 Metriken & Akzeptanzkriterien

- **M1:** AUD-Record-Vollständigkeit 100 %
- **M2:** Integritätsprüfungen bestanden
- **M3:** Abfragbarkeit nachgewiesen
...
## §6 Slot-Spezifikation AuditTrail — verbindliche Prüfkriterien
...
- **P5** (MUSS): Familienkonformitaet: AuditTrail / LogChain-Kernregeln KR-1..KR-6 eingehalten — Nachweis: Design-/Konzeptdokument + AUD-Record
...
- **REQ-STD-015** (§6/P5): Familienkonformitaet: AuditTrail / LogChain-Kernregeln KR-1..KR-6 eingehalten

## Security Considerations

Protokoll-Lücken verhindern Forensik; nachträgliche Protokoll-Änderung fälscht Nachweise.
...
- **1.0.0** (2026-09-08): Initial Release — Grundgerüst-Standard aus
  Katalog-Slot ATC-STD-640 (Familie AuditTrail / LogChain, FAM-36) via SCR-0030; §9-FREIGEGEBEN
...
- registry/framework.yaml (FAM-36), registry/standards.yaml + versions.yaml

*ATC-STD-640 v1.2.0 · Slot-Fertigbau via SCR-0034 · Aurora (Superagent) · 08.09.2026*
